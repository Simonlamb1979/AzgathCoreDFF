        ...
        else if (!strcmp(fourcc, "MODF"))
        {
            ...
            if (size)
            {
                ...
                for (uint32 i = 0; i < mapObjectCount; ++i)
                {
                    ...
                    {
                        std::string fileName = Azgath::StringFormat("FILE%08X.xxx", mapObjDef.Id);
...
ADTFile* WDTFile::GetMap(int32 x, int32 y)
{
    ...
    ADTFile* adt;
    std::string name = Azgath::StringFormat(R"(World\Maps\%s\%s_%d_%d_obj0.adt)", _mapName.c_str(), _mapName.c_str(), x, y);
