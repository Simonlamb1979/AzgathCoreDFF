...
char* DB2FileLoaderSparseImpl::AutoProduceData(uint32& indexTableSize, char**& indexTable)
{
    if (_loadInfo->Meta->FieldCount != _header->FieldCount)
        throw DB2FileLoadException(Azgath::StringFormat("Found unsupported parent index in sparse db2 %s", _fileName));
...
void DB2FileLoader::LoadHeaders(DB2FileSource* source, DB2FileLoadInfo const* loadInfo)
{
    ...
    if (_header.Signature != 0x33434457)                        //'WDC3'
        throw DB2FileLoadException(Azgath::StringFormat("Incorrect file signature in %s, expected 'WDC3', got %c%c%c%c", source->GetFileName(),
    ...
    if (loadInfo && _header.LayoutHash != loadInfo->Meta->LayoutHash)
        throw DB2FileLoadException(Azgath::StringFormat("Incorrect layout hash in %s, expected 0x%08X, got 0x%08X (possibly wrong client version)",
            source->GetFileName(), loadInfo->Meta->LayoutHash, _header.LayoutHash));

    if (_header.ParentLookupCount > 1)
        throw DB2FileLoadException(Azgath::StringFormat("Too many parent lookups in %s, only one is allowed, got %u",
    ...
    if (loadInfo && (_header.TotalFieldCount + (loadInfo->Meta->ParentIndexField >= int32(_header.TotalFieldCount) ? 1 : 0) != loadInfo->Meta->FieldCount))
        throw DB2FileLoadException(Azgath::StringFormat("Incorrect number of fields in %s, expected %u, got %u",
    ...
    if (loadInfo && (_header.ParentLookupCount && loadInfo->Meta->ParentIndexField == -1))
        throw DB2FileLoadException(Azgath::StringFormat("Unexpected parent lookup found in %s", source->GetFileName()));
    ...
    if (_header.SectionCount && !source->Read(sections.get(), sizeof(DB2SectionHeader) * _header.SectionCount))
        throw DB2FileLoadException(Azgath::StringFormat("Unable to read section headers from %s", source->GetFileName()));
    ...
    if (loadInfo && !(_header.Flags & 0x1))
    {
        ...
        if (source->GetFileSize() != expectedFileSize)
            throw DB2FileLoadException(Azgath::StringFormat("%s failed size consistency check, expected " SZFMTD ", got " SZFMTD, expectedFileSize, source->GetFileSize()));
    }
    ...
    if (!source->Read(fieldData.get(), sizeof(DB2FieldEntry) * _header.FieldCount))
        throw DB2FileLoadException(Azgath::StringFormat("Unable to read field information from %s", source->GetFileName()));
    ...
    if (_header.ColumnMetaSize)
    {
        ...
        if (!source->Read(columnMeta.get(), _header.ColumnMetaSize))
            throw DB2FileLoadException(Azgath::StringFormat("Unable to read field metadata from %s", source->GetFileName()));
        ...
        if (loadInfo && loadInfo->Meta->HasIndexFieldInData())
        {
            if (columnMeta[loadInfo->Meta->IndexField].CompressionType != DB2ColumnCompression::None
                ...
                throw DB2FileLoadException(Azgath::StringFormat("Invalid compression type for index field in %s, expected one of None (0), Immediate (1), SignedImmediate (5), got %u",
        ...
        for (uint32 i = 0; i < _header.TotalFieldCount; ++i)
        {
            ...
            if (!source->Read(palletValues[i].get(), columnMeta[i].AdditionalDataSize))
                throw DB2FileLoadException(Azgath::StringFormat("Unable to read field pallet values from %s for field %u", source->GetFileName(), i));
        }
        ...
        for (uint32 i = 0; i < _header.TotalFieldCount; ++i)
        {
            ...
            if (!source->Read(palletArrayValues[i].get(), columnMeta[i].AdditionalDataSize))
                throw DB2FileLoadException(Azgath::StringFormat("Unable to read field pallet array values from %s for field %u", source->GetFileName(), i));
        }
        ...
        for (uint32 i = 0; i < _header.TotalFieldCount; ++i)
        {
            ...
            if (!source->Read(commonData[i].get(), columnMeta[i].AdditionalDataSize))
                throw DB2FileLoadException(Azgath::StringFormat("Unable to read field common values from %s for field %u", source->GetFileName(), i));
...
void DB2FileLoader::Load(DB2FileSource* source, DB2FileLoadInfo const* loadInfo)
{
    ...
    for (uint32 i = 0; i < _header.SectionCount; ++i)
    {
        ...
        if (!source->SetPosition(section.FileOffset))
            throw DB2FileLoadException(Azgath::StringFormat("Unable to change %s read position for section %u", source->GetFileName(), i));

        if (!_impl->LoadTableData(source, i))
            throw DB2FileLoadException(Azgath::StringFormat("Unable to read section table data from %s for section %u", source->GetFileName(), i));

        if (!_impl->LoadCatalogData(source, i))
            throw DB2FileLoadException(Azgath::StringFormat("Unable to read section catalog data from %s for section %u", source->GetFileName(), i));
        ...
        if (loadInfo)
        {
            if (loadInfo->Meta->HasIndexFieldInData())
            {
                if (section.IdTableSize != 0)
                    throw DB2FileLoadException(Azgath::StringFormat("Unexpected id table found in %s for section %u", source->GetFileName(), i));
            }
            else if (section.IdTableSize != 4 * section.RecordCount)
                throw DB2FileLoadException(Azgath::StringFormat("Unexpected id table size in %s for section %u, expected %u, got %u",
        ...
        if (section.IdTableSize)
        {
            ...
            if (!source->Read(&idTable[idTableSize], section.IdTableSize))
                throw DB2FileLoadException(Azgath::StringFormat("Unable to read non-inline record ids from %s for section %u", source->GetFileName(), i));
        ...
        if (!(_header.Flags & 0x1) && section.CopyTableCount)
        {
            ...
            if (!source->Read(&copyTable[copyTableSize], section.CopyTableCount * sizeof(DB2RecordCopy)))
                throw DB2FileLoadException(Azgath::StringFormat("Unable to read record copies from %s for section %u", source->GetFileName(), i));
        }
        ...
        if (_header.ParentLookupCount)
        {
            ...
            for (uint32 j = 0; j < _header.ParentLookupCount; ++j)
            {
                ...
                if (!source->Read(&indexInfo, sizeof(DB2IndexDataInfo)))
                    throw DB2FileLoadException(Azgath::StringFormat("Unable to read parent lookup info from %s for section %u", source->GetFileName(), i));
                ...
                if (!source->Read(parentIndexesForSection[j].Entries.data(), sizeof(DB2IndexEntry) * indexInfo.NumEntries))
                    throw DB2FileLoadException(Azgath::StringFormat("Unable to read parent lookup content from %s for section %u", source->GetFileName(), i));
            }
        }
    }
