...
void Metric::Initialize(std::string const& realmName, Azgath::Asio::IoContext& ioContext, std::function<void()> overallStatusLogger)
{
    _dataStream = std::make_unique<boost::asio::ip::tcp::iostream>();
    _realmName = FormatInfluxDBTagValue(realmName);
    _batchTimer = std::make_unique<Azgath::Asio::DeadlineTimer>(ioContext);
    _overallStatusTimer = std::make_unique<Azgath::Asio::DeadlineTimer>(ioContext);
...
void Metric::LoadFromConfigs()
{
    ...
    if (_enabled && !previousValue)
    {
        ...
        std::vector<std::string_view> tokens = Azgath::Tokenize(connectionInfo, ';', true);
        if (tokens.size() != 3)
        {
...
void Metric::Unload()
{
    ...
    if (_enabled && Azgath::Asio::get_io_context(*_batchTimer).stopped())
    {
        _enabled = false;
        SendBatch();
    }
