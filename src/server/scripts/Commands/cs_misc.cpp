...
public:
    ...
    static bool HandleCooldownCommand(ChatHandler* handler, Optional<SpellInfo const*> spellArg)
    {
        ...
        else
        {
            ...
            handler->PSendSysMessage(LANG_REMOVE_COOLDOWN, (*spellArg)->Id, owner == handler->GetSession()->GetPlayer() ? handler->GetAzgathString(LANG_YOU) : nameLink.c_str());
        }
        return true;
    }
    ...
    **/
    static bool HandlePInfoCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (!result2)
        {
            ...
            banType = handler->GetAzgathString(LANG_ACCOUNT);
        ...
        if (banTime >= 0)
            handler->PSendSysMessage(LANG_PINFO_BANNED, banType.c_str(), banReason.c_str(), banTime > 0 ? secsToTimeString(banTime - GameTime::GetGameTime(), TimeFormat::ShortText).c_str() : handler->GetAzgathString(LANG_PERMANENTLY), bannedBy.c_str());
        ...
        if (!zoneName)
            zoneName = handler->GetAzgathString(LANG_UNKNOWN);
    ...
    static bool HandleMuteCommand(ChatHandler* handler, Optional<PlayerIdentifier> player, uint32 muteTime, Tail muteReason)
    {
        ...
        if (muteReason.empty())
            muteReasonStr = handler->GetAzgathString(LANG_NO_REASON);
