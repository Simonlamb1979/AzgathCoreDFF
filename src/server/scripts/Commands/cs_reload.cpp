...
public:
    ...
    static bool HandleReloadCreatureTemplateCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (std::string_view entryStr : Azgath::Tokenize(args, ' ', false))
        {
            uint32 entry = Azgath::StringTo<uint32>(entryStr).value_or(0);
    ...
    static bool HandleReloadAzgathStringCommand(ChatHandler* handler, char const* /*args*/)
    {
