...
public:
    ...
    static bool HandleBanInfoHelper(uint32 accountId, char const* accountName, ChatHandler* handler)
    {
        ...
        do
        {
            ...
            std::string banTime = permanent ? handler->GetAzgathString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt64(), TimeFormat::ShortText);
    ...
    static bool HandleBanInfoCharacterCommand(ChatHandler* handler, char const* args)
    {
        ...
        do
        {
            ...
            std::string banTime = permanent ? handler->GetAzgathString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetInt64(), TimeFormat::ShortText);
            handler->PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
                TimeToTimestampStr(fields[0].GetInt64()).c_str(), banTime.c_str(), active ? handler->GetAzgathString(LANG_YES) : handler->GetAzgathString(LANG_NO), fields[4].GetCString(), fields[5].GetCString());
        }
        while (result->NextRow());
    ...
    static bool HandleBanInfoIPCommand(ChatHandler* handler, char const* args)
    {
        ...
        handler->PSendSysMessage(LANG_BANINFO_IPENTRY,
            ...
            permanent ? handler->GetAzgathString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[3].GetUInt64(), TimeFormat::ShortText).c_str(), fields[4].GetCString(), fields[5].GetCString());
