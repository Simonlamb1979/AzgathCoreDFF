...
public:
    ...
    static bool HandleAHBotItemsAmountCommand(ChatHandler* handler, std::array<uint32, MAX_AUCTION_QUALITY> items)
    {
        ...
        for (AuctionQuality quality : EnumUtils::Iterate<AuctionQuality>())
            handler->PSendSysMessage(LANG_AHBOT_ITEMS_AMOUNT, handler->GetAzgathString(ahbotQualityLangIds.at(quality)), sAuctionBotConfig->GetConfigItemQualityAmount(quality));
    ...
    static bool HandleAHBotItemsAmountQualityCommand(ChatHandler* handler, uint32 amount)
    {
        ...
        handler->PSendSysMessage(LANG_AHBOT_ITEMS_AMOUNT, handler->GetAzgathString(ahbotQualityLangIds.at(Q)),
    ...
    static bool HandleAHBotStatusCommand(ChatHandler* handler, Optional<EXACT_SEQUENCE("all")> all)
    {
        ...
        if (all)
        {
            ...
            for (AuctionQuality quality : EnumUtils::Iterate<AuctionQuality>())
                handler->PSendSysMessage(fmtId, handler->GetAzgathString(ahbotQualityLangIds.at(quality)),
