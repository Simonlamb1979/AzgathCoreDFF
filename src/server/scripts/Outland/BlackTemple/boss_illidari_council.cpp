...
static uint32 GetRandomBossExcept(uint32 exception)
{
    ...
    return Azgath::Containers::SelectRandomContainerElement(bossData);
}
    ...
    void ExecuteEvent(uint32 eventId) override
    {
        switch (eventId)
        {
            case EVENT_BLESS:
            {
                ...
                Azgath::AnyFriendlyUnitInObjectRangeCheck checker(me, me, 100.0f);
                ...
                if (!TargetList.empty())
                {
                    Unit* target = Azgath::Containers::SelectRandomContainerElement(TargetList);
