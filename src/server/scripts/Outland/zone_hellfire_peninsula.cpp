        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_START:
                {
                    ...
                    {
                        Azgath::AllCreaturesOfEntryInRange pred(me, entry);
                        Azgath::CreatureListSearcher<Azgath::AllCreaturesOfEntryInRange> searcher(me, dummies, pred);
...
struct npc_infernal_rain_hellfire : public ScriptedAI
{
    ...
    void RebuildTargetList()
    {
        ...
        Azgath::AllCreaturesOfEntryInRange pred(me, NPC_INFERNAL_RAIN);
        Azgath::CreatureListSearcher<Azgath::AllCreaturesOfEntryInRange> searcher(me, others, pred);
        Cell::VisitAllObjects(me, searcher, 500.0f);
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = _events.ExecuteEvent())
        {
            switch (eventId)
            {
                case EVENT_CAST:
                {
                    if (Creature* target = ObjectAccessor::GetCreature(*me, Azgath::Containers::SelectRandomContainerElement(_targets)))
                    {
