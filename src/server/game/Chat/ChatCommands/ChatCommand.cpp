...
using ChatSubCommandMap = std::map<std::string_view, Azgath::Impl::ChatCommands::ChatCommandNode, StringCompareLessI_T>;

void Azgath::Impl::ChatCommands::ChatCommandNode::LoadFromBuilder(ChatCommandBuilder const& builder)
{
...
/*static*/ void Azgath::Impl::ChatCommands::ChatCommandNode::LoadCommandsIntoMap(ChatCommandNode* blank, ChatSubCommandMap& map, Azgath::ChatCommands::ChatCommandTable const& commands)
{
    for (ChatCommandBuilder const& builder : commands)
    {
        ...
        {
            std::vector<std::string_view> const tokens = Azgath::Tokenize(builder._name, COMMAND_DELIMITER, false);
...
/*static*/ ChatSubCommandMap const& Azgath::Impl::ChatCommands::ChatCommandNode::GetTopLevelMap()
{
    if (COMMAND_MAP.empty())
        LoadCommandMap();
    return COMMAND_MAP;
}
/*static*/ void Azgath::Impl::ChatCommands::ChatCommandNode::InvalidateCommandMap() { COMMAND_MAP.clear(); }
/*static*/ void Azgath::Impl::ChatCommands::ChatCommandNode::LoadCommandMap()
{
    ...
    if (PreparedQueryResult result = WorldDatabase.Query(WorldDatabase.GetPreparedStatement(WORLD_SEL_COMMANDS)))
    {
        do
        {
            ...
            for (std::string_view key : Azgath::Tokenize(name, COMMAND_DELIMITER, false))
            {
            ...
            if (std::holds_alternative<std::monostate>(cmd->_help))
                ...
                TC_LOG_ERROR("sql.sql", "Table `command` contains legacy help text for command '" STRING_VIEW_FMT "', which uses `azgath_string`. Skipped.", STRING_VIEW_FMT_ARG(name));
...
void Azgath::Impl::ChatCommands::ChatCommandNode::ResolveNames(std::string name)
{
...
void Azgath::Impl::ChatCommands::ChatCommandNode::SendCommandHelp(ChatHandler& handler) const
{
...
/*static*/ bool Azgath::Impl::ChatCommands::ChatCommandNode::TryExecuteCommand(ChatHandler& handler, std::string_view cmdStr)
{
...
/*static*/ void Azgath::Impl::ChatCommands::ChatCommandNode::SendCommandHelpFor(ChatHandler& handler, std::string_view cmdStr)
{
    ChatCommandNode const* cmd = nullptr;
    ChatSubCommandMap const* map = &GetTopLevelMap();
    for (std::string_view token : Azgath::Tokenize(cmdStr, COMMAND_DELIMITER, false))
    {
        FilteredCommandListIterator it1(*map, handler, token);
        if (!it1)
...
/*static*/ std::vector<std::string> Azgath::Impl::ChatCommands::ChatCommandNode::GetAutoCompletionsFor(ChatHandler const& handler, std::string_view cmdStr)
{
    std::string path;
    ChatCommandNode const* cmd = nullptr;
    ChatSubCommandMap const* map = &GetTopLevelMap();
    ...
    while (!oldTail.empty())
    {
        ...
        { /* ok, so it1 points at a partially matching subcommand - let's see if there are others */
            ...
            { /* there are multiple matching subcommands - terminate here and show possibilities */
                ...
                auto possibility = ([prefix = std::string_view(path), suffix = std::string_view(newTail)](std::string_view match)
                {
                    if (prefix.empty())
                    {
                        return Azgath::StringFormat(STRING_VIEW_FMT "%c" STRING_VIEW_FMT,
                            STRING_VIEW_FMT_ARG(match), COMMAND_DELIMITER, STRING_VIEW_FMT_ARG(suffix));
                    }
                    else
                    {
                        return Azgath::StringFormat(STRING_VIEW_FMT "%c" STRING_VIEW_FMT "%c" STRING_VIEW_FMT,
        ...
        {
            path = Azgath::StringFormat(STRING_VIEW_FMT "%c" STRING_VIEW_FMT,
    ...
    { /* there is some trailing text, leave it as is */
        ...
        { /* if we matched a command at some point, auto-complete it */
            return {
                Azgath::StringFormat(STRING_VIEW_FMT "%c" STRING_VIEW_FMT,
...
bool Azgath::Impl::ChatCommands::ChatCommandNode::IsInvokerVisible(ChatHandler const& who) const
{
    if (!_invoker)
        return false;
    if (who.IsConsole() && (_permission.AllowConsole == Azgath::ChatCommands::Console::No))
        return false;
    return who.HasPermission(_permission.RequiredPermission);
}

bool Azgath::Impl::ChatCommands::ChatCommandNode::HasVisibleSubCommands(ChatHandler const& who) const
{
    for (auto it = _subCommands.begin(); it != _subCommands.end(); ++it)
...
void Azgath::ChatCommands::LoadCommandMap() { Azgath::Impl::ChatCommands::ChatCommandNode::LoadCommandMap(); }
void Azgath::ChatCommands::InvalidateCommandMap() { Azgath::Impl::ChatCommands::ChatCommandNode::InvalidateCommandMap(); }
bool Azgath::ChatCommands::TryExecuteCommand(ChatHandler& handler, std::string_view cmd) { return Azgath::Impl::ChatCommands::ChatCommandNode::TryExecuteCommand(handler, cmd); }
void Azgath::ChatCommands::SendCommandHelpFor(ChatHandler& handler, std::string_view cmd) { Azgath::Impl::ChatCommands::ChatCommandNode::SendCommandHelpFor(handler, cmd); }
std::vector<std::string> Azgath::ChatCommands::GetAutoCompletionsFor(ChatHandler const& handler, std::string_view cmd) { return Azgath::Impl::ChatCommands::ChatCommandNode::GetAutoCompletionsFor(handler, cmd); }
