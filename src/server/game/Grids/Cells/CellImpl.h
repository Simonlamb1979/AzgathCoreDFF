...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellCoord center = Azgath::ComputeCellCoord(x, y).normalize();
        return CellArea(center, center);
    }

    CellCoord centerX = Azgath::ComputeCellCoord(x - radius, y - radius).normalize();
    CellCoord centerY = Azgath::ComputeCellCoord(x + radius, y + radius).normalize();
...
{
    CellCoord p(Azgath::ComputeCellCoord(center_obj->GetPositionX(), center_obj->GetPositionY()));
    Cell cell(p);
    if (dont_load)
        cell.SetNoCreate();
