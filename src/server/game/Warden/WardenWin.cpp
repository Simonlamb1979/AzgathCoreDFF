...
WardenWin::WardenWin() : Warden(), _serverTicks(0)
{
    for (WardenCheckCategory category : EnumUtils::Iterate<WardenCheckCategory>())
    {
        ...
        Azgath::Containers::RandomShuffle(checks);
...
void WardenWin::Init(WorldSession* session, SessionKey const& K)
{
    _session = session;
    // Generate Warden Key
    SessionKeyGenerator<Azgath::Crypto::SHA1> WK(K);
...
{
    // Verify key
    Azgath::Crypto::SHA1::Digest response;
    buff.read(response);
...
static constexpr uint8 GetCheckPacketBaseSize(WardenCheckType type)
{
    switch (type)
    {
        ...
        case MODULE_CHECK: return (4 + Azgath::Crypto::HMAC_SHA1::DIGEST_LENGTH);
...
void WardenWin::RequestChecks()
{
    ...
    Azgath::Containers::RandomShuffle(_currentChecks);

    uint16 expectedSize = 4;
    Azgath::Containers::EraseIf(_currentChecks,
    ...
    for (uint16 const id : _currentChecks)
    {
        ...
        switch (type)
        {
            ...
            case MODULE_CHECK:
            {
                std::array<uint8, 4> seed = Azgath::Crypto::GetRandomBytes<4>();
                buff.append(seed);
                buff.append(Azgath::Crypto::HMAC_SHA1::GetDigestOf(seed, check.Str));
                break;
            }
            /*case PROC_CHECK:
            {
    ...
    for (uint16 const id : _currentChecks)
    {
        ...
        switch (check.Type)
        {
            ...
            case MPQ_CHECK:
            {
                ...
                result.resize(Azgath::Crypto::SHA1::DIGEST_LENGTH);
