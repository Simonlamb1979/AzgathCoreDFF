...
bool PlayerTaxi::LoadTaxiMask(std::string const& data)
{
    ...
    {
        if (Optional<uint32> mask = Azgath::StringTo<uint32>(tokens[index]))
        {
...
bool PlayerTaxi::LoadTaxiDestinationsFromString(const std::string& values, uint32 team)
{
    ...
    std::vector<std::string_view> tokens = Azgath::Tokenize(values, ' ', false);
    auto itr = tokens.begin();
    if (itr != tokens.end())
    {
        if (Optional<uint32> faction = Azgath::StringTo<uint32>(*itr))
    ...
    {
        if (Optional<uint32> node = Azgath::StringTo<uint32>(*itr))
